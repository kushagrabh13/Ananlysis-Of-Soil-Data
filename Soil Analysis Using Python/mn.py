...
##df = df[df['Mn(mgkg-1)'] < 50]
##df = df[df['Mn(mgkg-1)'] > 20]
...
#Plot of Mn variation along LAT and LONG

fig0 = plt.figure(figsize=(20,10))
fig0.canvas.set_window_title('Variation Of Mn with Latitude and Longitude')
plt.subplots_adjust(hspace = 0.5)
plt.subplot(2, 1, 1)
plt.scatter(X[:,0] , y)
plt.title('Variation of Mn with Latitude')
plt.xlabel('Latitude')
plt.ylabel('Mn')

plt.subplot(2, 1, 2)
plt.scatter(X[:,1] , y)
plt.title('Variation of Mn with Longitude')
plt.xlabel('Longitude')
plt.ylabel('Mn(mgkg-1)')
...
fig1.canvas.set_window_title('Hyperplane for predicting Mn')
plt.clf()
