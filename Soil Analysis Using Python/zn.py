...
##df = df[df['Zn(mgkg-1)'] < 1.25]
##df = df[df['Zn(mgkg-1)'] > 0.3]
...
#Plot of Zn variation along LAT and LONG

fig0 = plt.figure(figsize=(20,10))
fig0.canvas.set_window_title('Variation Of Zn with Latitude and Longitude')
plt.subplots_adjust(hspace = 0.5)
plt.subplot(2, 1, 1)
plt.scatter(X[:,0] , y)
plt.title('Variation of Zn with Latitude')
plt.xlabel('Latitude')
plt.ylabel('Zn')

plt.subplot(2, 1, 2)
plt.scatter(X[:,1] , y)
plt.title('Variation of Zn with Longitude')
plt.xlabel('Longitude')
plt.ylabel('Zn(mgkg-1)')
...
fig1.canvas.set_window_title('Hyperplane for predicting Zn')
plt.clf()
...
for i in range(elev.size):
    elev_scaled = np.append(elev_scaled, (((elev[i]-elev.min())*(4))/(elev.max()-elev.min())) + (0))
...
ax1.set_zlabel('Elevation From Mean Sea Level (Scaled Down to 0-4) ')
