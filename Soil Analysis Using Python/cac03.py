...
##df = df[df['CaCO3(gkg-1)'] < 100]
##df = df[df['CaCO3(gkg-1)'] > 20]
...
sq_errors = sq_errors.sort(0)
print('R^2 Score : ',reg.score(X_test,y_test))
accuracy = (np.sum(1-abs((y_test-y_pred)/y_test))/(y_test.size))*100
print('Acuracy : ',accuracy)

#Plot of CaCO3 variation along LAT and LONG

fig0 = plt.figure(figsize=(20,10))
fig0.canvas.set_window_title('Variation Of CaCO3 with Latitude and Longitude')
plt.subplots_adjust(hspace = 0.5)
plt.subplot(2, 1, 1)
plt.scatter(X[:,0] , y)
plt.title('Variation of CaCO3 with Latitude')
plt.xlabel('Latitude')
plt.ylabel('CaCO3')

plt.subplot(2, 1, 2)
plt.scatter(X[:,1] , y)
plt.title('Variation of CaCO3 with Longitude')
plt.xlabel('Longitude')
plt.ylabel('CaCO3(gkg-1)')
...
fig1.canvas.set_window_title('Hyperplane for predicting CaCO3')
plt.clf()
...
for i in range(elev.size):
    elev_scaled = np.append(elev_scaled, (((elev[i]-elev.min())*(120))/(elev.max()-elev.min())) + (0))
...
ax1.set_zlabel('Elevation From Mean Sea Level (Scaled Down to 0-120)')
