...
##df = df[df['EC(dSm-1)'] < 0.2]
##df = df[df['EC(dSm-1)'] > 0.04]
...
#Plot of EC variation along LAT and LONG

fig0 = plt.figure(figsize=(20,10))
fig0.canvas.set_window_title('Variation Of EC with Latitude and Longitude')
plt.subplots_adjust(hspace = 0.5)
plt.subplot(2, 1, 1)
plt.scatter(X[:,0] , y)
plt.title('Variation of EC with Latitude')
plt.xlabel('Latitude')
plt.ylabel('EC(mgkg-1)')

plt.subplot(2, 1, 2)
plt.scatter(X[:,1] , y)
plt.title('Variation of EC with Longitude')
plt.xlabel('Longitude')
plt.ylabel('EC(dSkg-1)')
...
fig1.canvas.set_window_title('Hyperplane for predicting EC')
plt.clf()
...
for i in range(elev.size):
    elev_scaled = np.append(elev_scaled, (((elev[i]-elev.min())*(1))/(elev.max()-elev.min())) + (0))
...
ax1.set_zlabel('Elevation From Mean Sea Level (Scaled Down to 0-1) ')
