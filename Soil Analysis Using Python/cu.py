...
##df = df[df['Cu(mgkg-1)'] < 2.25]
##df = df[df['Cu(mgkg-1)'] > 0.3]
...
plt.subplot(2, 1, 2)
plt.scatter(X[:,1] , y)
plt.title('Variation of Cu with Longitude')
plt.xlabel('Longitude')
plt.ylabel('Cu(mgkg-1)')
...
fig1.canvas.set_window_title('Hyperplane for predicting Cu')
plt.clf()
...
for i in range(elev.size):
    elev_scaled = np.append(elev_scaled, (((elev[i]-elev.min())*(6))/(elev.max()-elev.min())) + (0))
...
ax1.set_zlabel('Elevation From Mean Sea Level (Scaled Down to 0-6) ')
